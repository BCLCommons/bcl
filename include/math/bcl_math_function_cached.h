...
#ifndef BCL_MATH_FUNCTION_CACHED_H_
#define BCL_MATH_FUNCTION_CACHED_H_
...
namespace bcl
{
  namespace math
  {
    ...
    //! @details The FunctionCached class wraps a around a normal FunctionInterface derived class, and stores results
    ...
    //!
    //! @see @link example_math_function_cached.cpp @endlink
    //! @author woetzen
    //! @date Jun 4, 2010
    //!
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    //////////
      ...
      util::ShPtr< FunctionInterfaceSerializable< t_ArgumentType, t_ResultType> > m_Function;
      ...
      //! destructor signal handler which FunctionCached connects to on the Argument
      SignalHandlerFunctionPtr m_DestructorHandlerToConnectTo;

      //! set of signal handlers wo which FunctionCached connects to on the Argument
    ...
    //////////////////////////////////
      ...
      FunctionCached
      (
        const util::ShPtr< FunctionInterfaceSerializable< t_ArgumentType, t_ResultType> > &SP_FUNCTION,
        SignalHandlerFunctionPtr DESTRUCTOR_SIGNAL_HANDLER,
        SignalHandlerFunctionPtr CHANGE_SIGNAL_HANDLER = NULL
      ) :
        m_Function( SP_FUNCTION),
        m_DestructorHandlerToConnectTo( DESTRUCTOR_SIGNAL_HANDLER),
        m_HandlersToConnectTo()
      {
        if( CHANGE_SIGNAL_HANDLER)
        {
          m_HandlersToConnectTo.push_back( CHANGE_SIGNAL_HANDLER);
        }
      }
      ...
      FunctionCached
      (
        const FunctionCached< t_ArgumentType, t_ResultType> &FUNCTION_CACHED_RHS
      ) :
        signal::Slots(),
      ...
      //! @return pointer to new FunctionCached< t_ArgumentType, t_ResultType>
      FunctionCached< t_ArgumentType, t_ResultType> *Clone() const
      {
        return new FunctionCached< t_ArgumentType, t_ResultType>( *this);
      }
    ...
    /////////////////
      ...
      const util::ShPtr< FunctionInterfaceSerializable< t_ArgumentType, t_ResultType> > &GetFunction() const
      {
        return m_Function;
      }
    ...
    ///////////////
      ...
      //! will not copy the actual cache and signal connection to already seen arguments
      //! @param FUNCTION_CACHED_RHS cache function to copy
      //! @return reference to this FunctionCached< t_ArgumentType, t_ResultType> object
      FunctionCached< t_ArgumentType, t_ResultType> &operator =
      (
        const FunctionCached< t_ArgumentType, t_ResultType> &FUNCTION_CACHED_RHS
      )
      {
    ...
    private:
      ...
      {
        FunctionCached< t_ArgumentType, t_ResultType> *non_const_this_ptr
        (
          const_cast< FunctionCached< t_ArgumentType, t_ResultType> *>( this)
        );
        ...
        (
          non_const_this_ptr, &FunctionCached< t_ArgumentType, t_ResultType>::RemoveResultFromCache
        );
        ...
        )
        {
          ...
          (
            non_const_this_ptr, &FunctionCached< t_ArgumentType, t_ResultType>::InvalidateResult
          );
        }
      }

    }; // template class FunctionCached

    // instantiate s_Instance
    template< typename t_ArgumentType, typename t_ResultType>
    const util::SiPtr< const util::ObjectInterface> FunctionCached< t_ArgumentType, t_ResultType>::s_Instance
    (
      GetObjectInstances().AddInstance
      (
        FunctionCached< t_ArgumentType, t_ResultType>( util::ShPtr< FunctionInterfaceSerializable< t_ArgumentType, t_ResultType> >())
      )
    );
...
#endif // BCL_MATH_FUNCTION_CACHED_H_
