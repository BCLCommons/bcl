...
#ifndef BCL_MATH_BINARY_FUNCTION_CACHED_H_
#define BCL_MATH_BINARY_FUNCTION_CACHED_H_
...
namespace bcl
{
  namespace math
  {
    ...
    //! @details The BinaryFunctionCached class wraps a around a normal BinaryFunctionInterface derived class, and
    ...
    //!
    //! @see @link example_math_binary_function_cached.cpp @endlink
    //! @author woetzen
    //! @date Jun 4, 2010
    //!
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    //////////
      ...
      util::ShPtr< BinaryFunctionInterface< t_ArgumentType1, t_ArgumentType2, t_ResultType> > m_Function;
      ...
      //! destructor signal handler which BinaryFunctionCached connects to on the t_ArgumentType1
      SignalHandlerFunctionPtr1 m_DestructorHandlerToConnectTo1;

      //! destructor signal handler which BinaryFunctionCached connects to on the t_ArgumentType2
      SignalHandlerFunctionPtr2 m_DestructorHandlerToConnectTo2;

      //! set of signal handlers  which BinaryFunctionCached connects to on the t_ArgumentType1
      std::list< SignalHandlerFunctionPtr1> m_HandlersToConnectTo1;

      //! set of signal handlers which BinaryFunctionCached connects to on the t_ArgumentType2
    ...
    //////////////////////////////////
      ...
      BinaryFunctionCached
      (
        const util::ShPtr< BinaryFunctionInterfaceSerializable< t_ArgumentType1, t_ArgumentType2, t_ResultType> > &SP_FUNCTION,
      ...
      BinaryFunctionCached
      (
        const BinaryFunctionCached< t_ArgumentType1, t_ArgumentType2, t_ResultType> &FUNCTION_CACHED_RHS
      ) :
        signal::Slots(),
      ...
      //! @return pointer to new BinaryFunctionCached< t_ArgumentType1, t_ArgumentType2, t_ResultType>
      BinaryFunctionCached< t_ArgumentType1, t_ArgumentType2, t_ResultType> *Clone() const
      {
        return new BinaryFunctionCached< t_ArgumentType1, t_ArgumentType2, t_ResultType>( *this);
      }
    ...
    /////////////////
      ...
      const util::ShPtr< BinaryFunctionInterface< t_ArgumentType1, t_ArgumentType2, t_ResultType> > &GetFunction() const
      {
        return m_Function;
      }
    ...
    ///////////////
      ...
      //! will not copy the actual cache and signal connection to already seen arguments
      //! @param FUNCTION_CACHED_RHS cache function to copy
      //! @return reference to this BinaryFunctionCached< t_ArgumentType, t_ResultType> object
      BinaryFunctionCached< t_ArgumentType1, t_ArgumentType2, t_ResultType> &operator =
      (
        const BinaryFunctionCached< t_ArgumentType1, t_ArgumentType2, t_ResultType> &FUNCTION_CACHED_RHS
      )
      {
    ...
    //////////////////////
      ...
      {
        return m_Function->WriteDetailedSchemeAndValues( ARGUMENT1, ARGUMENT2, OSTREAM);
      }
    ...
    private:
      ...
      {
        BinaryFunctionCached< t_ArgumentType1, t_ArgumentType2, t_ResultType> *non_const_this_ptr
        (
          const_cast< BinaryFunctionCached< t_ArgumentType1, t_ArgumentType2, t_ResultType> *>( this)
        );
        ...
        (
          non_const_this_ptr, &BinaryFunctionCached< t_ArgumentType1, t_ArgumentType2, t_ResultType>::RemoveResultFromCache1
        );
        ...
        )
        {
          ...
          (
            non_const_this_ptr, &BinaryFunctionCached< t_ArgumentType1, t_ArgumentType2, t_ResultType>::InvalidateResult1
          );
        }
      }
      ...
      void ConnectToAllSignalHandlers2( const t_ArgumentType2 &ARGUMENT) const
      {
        ...
        (
          non_const_this_ptr, &BinaryFunctionCached< t_ArgumentType1, t_ArgumentType2, t_ResultType>::RemoveResultFromCache2
        );
        ...
        )
        {
          (ARGUMENT.**func_itr)().Connect
          (
            non_const_this_ptr, &BinaryFunctionCached< t_ArgumentType1, t_ArgumentType2, t_ResultType>::InvalidateResult2
          );
        }
      }

    }; // template class BinaryFunctionCached

    // instantiate s_Instance
    template< typename t_ArgumentType1, typename t_ArgumentType2, typename t_ResultType>
    const util::SiPtr< const util::ObjectInterface> BinaryFunctionCached< t_ArgumentType1, t_ArgumentType2, t_ResultType>::s_Instance
    (
      GetObjectInstances().AddInstance
      (
        new BinaryFunctionCached< t_ArgumentType1, t_ArgumentType2, t_ResultType>( util::ShPtr< BinaryFunctionInterface< t_ArgumentType1, t_ArgumentType2, t_ResultType> >())
      )
    );

    //! @brief partial specialization of BinaryFunctionCached for the special case t_ArgumentType1 = t_ArgumentType2
    template< typename t_ArgumentType, typename t_ResultType>
    class BinaryFunctionCached< t_ArgumentType, t_ArgumentType, t_ResultType> :
      public BinaryFunctionInterfaceSerializable< t_ArgumentType, t_ArgumentType, t_ResultType>,
      public signal::Slots
    {
    ...
    //////////
      ...
      //! @brief the actual function
      util::ShPtr< BinaryFunctionInterface< t_ArgumentType, t_ArgumentType, t_ResultType> > m_Function;
      ...
      //! the key is the pointer (address) of the argument, the data is a pointer to a result, NULL if the result does
      //! not exist (because the argument has changed)
      typedef std::map< const t_ArgumentType *, const t_ResultType *> InteriorCacheType;
      typedef std::map< const t_ArgumentType *, InteriorCacheType> CacheType;
      mutable CacheType m_Cache;
      //! Map from each argument to iterators to all places in the cache where this is the second argument
      mutable std::map
      <
        const t_ArgumentType *,
        std::list< std::pair< typename CacheType::iterator, typename InteriorCacheType::iterator> >
      > m_ArgToValues;
      ...
      //! destructor signal handler which BinaryFunctionCached connects to on the t_ArgumentType
      SignalHandlerFunctionPtr m_DestructorHandlerToConnectTo;

      //! set of signal handlers which BinaryFunctionCached connects to on the t_ArgumentType
    ...
    //////////////////////////////////
      ...
      BinaryFunctionCached
      (
        const util::ShPtr< BinaryFunctionInterface< t_ArgumentType, t_ArgumentType, t_ResultType> > &SP_FUNCTION,
      ...
      BinaryFunctionCached
      (
        const BinaryFunctionCached< t_ArgumentType, t_ArgumentType, t_ResultType> &FUNCTION_CACHED_RHS
      ) :
        signal::Slots(),
        m_Function( FUNCTION_CACHED_RHS.m_Function),
      ...
      //! @brief Clone function
      //! @return pointer to new BinaryFunctionCached< t_ArgumentType, t_ArgumentType2, t_ResultType>
      BinaryFunctionCached< t_ArgumentType, t_ArgumentType, t_ResultType> *Clone() const
      {
        return new BinaryFunctionCached< t_ArgumentType, t_ArgumentType, t_ResultType>( *this);
      }
      ...
      ~BinaryFunctionCached()
      {
        ...
        for( typename CacheType::iterator itr( copy.begin()), itr_end( copy.end()); itr != itr_end; ++itr)
        {
          for
          (
            typename InteriorCacheType::iterator itr_i( itr->second.begin()), itr_i_end( itr->second.end());
            itr_i != itr_i_end;
            ++itr_i
          )
          {
            // delete only if there is an actual object
            if( itr_i->second != NULL)
            {
              delete itr_i->second;
              itr_i->second = NULL;
            }
    ...
    /////////////////
      ...
      //! @brief access to function
      const util::ShPtr< BinaryFunctionInterface< t_ArgumentType, t_ArgumentType, t_ResultType> > &GetFunction() const
      {
        return m_Function;
      }
    ...
    ///////////////
      ...
      //! @param ARGUMENT2 Argument2 to be used to evaluate the function
      //! @return function value of the given argument
      t_ResultType operator()( const t_ArgumentType &ARGUMENT1, const t_ArgumentType &ARGUMENT2) const
      {
        // order pair by address, if symmetric
        if( m_Symmetric && &ARGUMENT1 > &ARGUMENT2)
        {
          return operator()( ARGUMENT2, ARGUMENT1);
        }

        // search in cache
        typename CacheType::iterator itr( m_Cache.find( &ARGUMENT1));
        ...
        if( itr == m_Cache.end())
        {
          ...
          // check that insert was successful
          BCL_Assert( insert_pair.second, "could not insert the result!");
          itr = insert_pair.first;
        }

        // search in cache
        typename InteriorCacheType::iterator itr_i( itr->second.find( &ARGUMENT2));

        if( itr_i == itr->second.end())
        {
          // try to insert
          const std::pair< typename InteriorCacheType::iterator, bool> insert_pair
          (
            itr->second.insert
            (
              typename InteriorCacheType::value_type
              (
                &ARGUMENT2,
                new t_ResultType( m_Function->operator()( ARGUMENT1, ARGUMENT2))
              )
          ...
          m_ArgToValues[ &ARGUMENT2].push_back
          (
            std::pair< typename CacheType::iterator, typename InteriorCacheType::iterator>( itr, insert_pair.first)
          );
        ...
        else
        {
          ...
          if( itr_i->second == NULL)
          {
            itr_i->second = new t_ResultType( m_Function->operator()( ARGUMENT1, ARGUMENT2));
          }
      ...
      //! @param FUNCTION_CACHED_RHS cache function to copy
      //! @return reference to this BinaryFunctionCached< t_ArgumentType, t_ResultType> object
      BinaryFunctionCached< t_ArgumentType, t_ArgumentType, t_ResultType> &operator =
      (
        const BinaryFunctionCached< t_ArgumentType, t_ArgumentType, t_ResultType> &FUNCTION_CACHED_RHS
      )
      {
    ...
    private:
      ...
      void RemoveResultFromCache( const t_ArgumentType &ARGUMENT)
      {
        ...
        // collect all results to delete - otherwise deleting a result could trigger another call to this function
        // causing the iterator on the map to be invalidated
        std::list< const t_ResultType *> results_to_delete;

        // find result for that Argument
        typename CacheType::iterator itr( m_Cache.find( ptr));
        if( itr != m_Cache.end())
        {
          for( auto itr_i( itr->second.begin()), itr_i_end( itr->second.end()); itr_i != itr_i_end; ++itr_i)
          {
            results_to_delete.push_back( itr_i->second);
            auto &list_ptr( m_ArgToValues.find( itr_i->first)->second);
            for( auto itr_ii( list_ptr.begin()), itr_ii_end( list_ptr.end()); itr_ii != itr_ii_end; ++itr_ii)
            {
              if( itr_ii->second == itr_i)
              {
                list_ptr.erase( itr_ii);
                break;
              }
            }
          }
        ...
        auto itrb( m_ArgToValues.find( ptr));
        if( itrb != m_ArgToValues.end())
        {
          for( auto itr_i( itrb->second.begin()), itr_i_end( itrb->second.end()); itr_i != itr_i_end; ++itr_i)
          {
            results_to_delete.push_back( itr_i->second->second);
            itr_i->first->second.erase( itr_i->second);
          }
          m_ArgToValues.erase( itrb);
        }

        // delete all results that need deletion
        for
        (
          typename std::list< const t_ResultType *>::iterator
            itr( results_to_delete.begin()), itr_end( results_to_delete.end());
          itr != itr_end;
          ++itr
        )
      ...
      void InvalidateResult( const t_ArgumentType &ARGUMENT)
      {
        ...
        if( itr != m_Cache.end())
        {
          for( auto itr_i( itr->second.begin()), itr_i_end( itr->second.end()); itr_i != itr_i_end; ++itr_i)
          {
            const t_ResultType *result( itr_i->second);
            itr_i->second = NULL;
            delete result;
          }
        }
        ...
        if( itrb != m_ArgToValues.end())
        {
          for( auto itr_i( itrb->second.begin()), itr_i_end( itrb->second.end()); itr_i != itr_i_end; ++itr_i)
          {
            const t_ResultType *result( itr_i->second->second);
            itr_i->second->second = NULL;
            delete result;
          }
        }
      }
      ...
      {
        BinaryFunctionCached< t_ArgumentType, t_ArgumentType, t_ResultType> *non_const_this_ptr
        (
          const_cast< BinaryFunctionCached< t_ArgumentType, t_ArgumentType, t_ResultType> *>( this)
        );
        ...
        (
          non_const_this_ptr,
          &BinaryFunctionCached< t_ArgumentType, t_ArgumentType, t_ResultType>::RemoveResultFromCache
        );
        ...
        )
        {
          (ARGUMENT.**func_itr)().Connect
          (
            non_const_this_ptr, &BinaryFunctionCached< t_ArgumentType, t_ArgumentType, t_ResultType>::InvalidateResult
          );
        }
      }
...
#endif // BCL_MATH_BINARY_FUNCTION_CACHED_H_
