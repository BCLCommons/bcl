...
#ifndef BCL_ALIGN_ALIGNMENT_NODE_H_
#define BCL_ALIGN_ALIGNMENT_NODE_H_
...
namespace bcl
{
  namespace align
  {
    ...
    //!
    //! @class AlignmentNode
    //! @brief a node alignment in the alignment tree; has no sequences and 2..n parent alignments
    //!
    //! @tparam t_Member type of elements of a sequence and an assignment
    //!
    //! @see @link example_align_alignment_node.cpp @endlink
    //! @author heinzes1
    //! @date Sep 28, 2010
    //!
    //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    //////////////////////////////////
      ...
      AlignmentNode( const util::ShPtrList< AlignmentInterface< t_Member> > &SP_CHILD_ALIGNMENT_LIST) :
      ...
      AlignmentNode
      (
        util::ShPtr< AlignmentInterface< t_Member> > &SP_CHILD_ALIGNMENT_A,
        util::ShPtr< AlignmentInterface< t_Member> > &SP_CHILD_ALIGNMENT_B
      ) :
        m_ChildAlignments(),
        m_Assignments()
      {
      ...
      //! @param ALIGNMENT alignment of type AlignmentInterface to be converted into an AlignmentNode type
      AlignmentNode( const AlignmentInterface< t_Member> &ALIGNMENT) :
        m_ChildAlignments( ALIGNMENT.GetChildAlignments()),
        m_Assignments( ALIGNMENT.GetAssignments())
      {
        if( m_ChildAlignments.GetSize() == 0) // expand a Leaf into a node with a subtree behind not supported (yet)
        {
          ResetAssignments();
          BCL_MessageStd
          (
            "Leaf(" + util::Format()( ALIGNMENT.GetDepth()) + ") -> Node conversion not supported."
          );
        }
      }

      //! @brief Clone function
      //! @return pointer to new AlignmentNode< t_Member>
      AlignmentNode< t_Member> *Clone() const
      {
        return new AlignmentNode< t_Member>( *this);
      }

      //! @brief virtual empty constructor maintaining the child alignment list while resetting the assignment list
      //! @return pointer to new AlignmentNode< t_Member>
      AlignmentNode< t_Member> *Empty() const
      {
        return new AlignmentNode< t_Member>( m_ChildAlignments);
      }
    ...
    }; // template class AlignmentNode

    // instantiate s_Instance
    template< typename t_Member>
    const util::SiPtr< const util::ObjectInterface> AlignmentNode< t_Member>::s_Instance
    (
      GetObjectInstances().AddInstance( new AlignmentNode< t_Member>())
    );

  } // namespace align
} // namespace bcl

#endif // BCL_ALIGN_ALIGNMENT_NODE_H_
