...
#ifndef BCL_OPENCL_ARG_MIN_H_
#define BCL_OPENCL_ARG_MIN_H_
...
namespace bcl
{
  namespace opencl
  {
    ...
    //!
    //! @class ArgMin
    //! @brief gets the min value in an array and returns it as well as its index. Optimized for the gpu
    //!
    //! @tparam t_DataType can be float, double, complex, int, etc...
    //!
    //! @see @link example_opencl_arg_min.cpp @endlink
    //! @author loweew
    //! @date Mar 24, 2011
    //!
    //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    //////////////////////////////////
      ...
      //! @brief default constructor
      ArgMin( const CommandQueue &QUEUE) :
        m_Queue( QUEUE),
        m_Program()
      {
      ...
      //! @return pointer to new ArgMin
      ArgMin< t_DataType> *Clone() const
      {
        return new ArgMin< t_DataType>( *this);
      }
    ...
    ///////////////
      ...
      //! @brief operator for performing arg min
      //! @param DATA data for which to find min and index of min
      //! @return the resulting min value and its index
      storage::Pair< t_DataType, int> operator()( Vector< t_DataType> &DATA) const
      {
        ...
        cl::Kernel kernel( m_Program, "ArgMin", &error_number);
        ...
        // complete on cpu
        t_DataType final_element( std::numeric_limits< t_DataType>::infinity());
        size_t final_index( 0);
        for( size_t count( 0); count < nr_groups; ++count)
        {
          size_t less_than( elements( count) < final_element ? 1 : 0);
          less_than ? final_element = elements( count), final_index = indeces( count) : 0;
        }
    ...
    }; // template class ArgMin

  } // namespace opencl
} // namespace bcl

#endif // BCL_OPENCL_ARG_MIN_H_ 
