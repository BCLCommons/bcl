...
#include "bcl_descriptor_numeric.h"
...
namespace bcl
{
  ...
  //////////////////////////////////
    ...
    Numeric< t_DataType>::Numeric()
    : m_DescriptorSize( size_t( 1))
    {
    }

    //! @brief virtual copy constructor
    template< typename t_DataType>
    Numeric< t_DataType> *Numeric< t_DataType>::Clone() const
    {
  ...
  /////////////////
    ...
    template< typename t_DataType>
    const std::string &Numeric< t_DataType>::GetClassIdentifier() const
    {
    ...
    template< typename t_DataType>
    const std::string &Numeric< t_DataType>::GetAlias() const
    {
      static const std::string s_name( "ToNumeric");
      return s_name;
    }
    ...
    template< typename t_DataType>
    size_t Numeric< t_DataType>::GetNormalDimension() const
    {
    ...
    template< typename t_DataType>
    Type::Symmetry Numeric< t_DataType>::GetSymmetry() const
    {
    ...
    template< typename t_DataType>
    bool Numeric< t_DataType>::ConsiderRepeatedElements() const
    {
    ...
    template< typename t_DataType>
    size_t Numeric< t_DataType>::GetNormalSizeOfFeatures() const
    {
  ...
  //////////////////////
    ...
    template< typename t_DataType>
    io::Serializer Numeric< t_DataType>::GetSerializer() const
    {
      ...
      (
        ...
        io::Serialization::GetAgent( &m_DescriptorSize),
        "1"
      );
  ...
  ////////////////
    ...
    template< typename t_DataType>
    void Numeric< t_DataType>::SetObjectHook()
    {
    ...
    template< typename t_DataType>
    void Numeric< t_DataType>::SetDimensionHook()
    {
    ...
    //! dimension
    template< typename t_DataType>
    void Numeric< t_DataType>::RecalculateImpl
    (
    ...
    )
    {
      ...
      const linal::VectorConstReference< char> key_linal( m_NumericProperty->operator()( ITR));
      ...
      linal::VectorReference< float>::iterator itr_storage( STORAGE.Begin());
      storage::Vector< float> key_vec( util::SplitStringToNumerical< float>( std::string( key_linal.Begin(), key_linal.GetSize())));
      BCL_Assert
      (
        key_vec.GetSize() == m_DescriptorSize, "Descriptor size is not of expected size"
      );
      for( storage::Vector< float>::const_iterator itr( key_vec.Begin()), itr_end( key_vec.End()); itr != itr_end; ++itr, ++itr_storage)
      {
        *itr_storage = float( *itr);
      }
    }
