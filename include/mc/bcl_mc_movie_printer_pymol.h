...
#ifndef BCL_MC_MOVIE_PRINTER_PYMOL_H_
#define BCL_MC_MOVIE_PRINTER_PYMOL_H_
...
namespace bcl
{
  namespace mc
  {
    ...
    //! @brief creates a movie script for pymol
    //!
    //! @see @link example_mc_movie_printer_pymol.cpp @endlink
    //! @author woetzen, weinerbe
    //! @date Jul 21, 2009
    //!
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

    class BCL_API MoviePrinterPymol :
    ...
    //////////
      ...
      std::string m_Prefix; //! prefix for files

      bool m_HasStartFrame; //! has start frame

      storage::Vector< std::string>  m_LabelColNames; //!< column names
      storage::Vector< std::string>  m_LabelRowNames; //!< row names
      std::vector< util::Format>     m_LabelFormats;  //!< formatting for cols
      ...
      std::string m_WaterMark;   //!< watermark text

      static const size_t s_WaitTimes[]; //!< wait times depending on step status
      static const std::string s_ColorFunction; //!< pymol function to color by residue id
      static const size_t s_TableWidth; //!< table width in pixels
    ...
    //////////////////////
      ...
      //! @param PNG_NAME
      //! @return string
      std::string TableToLabel( const storage::Table< double> &TABLE, const std::string &PNG_NAME) const;

      //! @brief Write the step status legend
      //! @param STEP_STATUS given step status
      //! @param PNG_NAME
      //! @return the step status legend
      std::string WriteLegend( const opti::StepStatus &STEP_STATUS, const std::string &PNG_NAME) const;

      //! @brief Write the watermark
      //! @param PNG_NAME
      //! @return the watermark
      std::string WriteWaterMark( const std::string &PNG_NAME);
...
#endif // BCL_MC_MOVIE_PRINTER_PYMOL_H_ 
