...
namespace bcl
{
  //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  //!
  //! @example example_chemistry_conformation_comparison_property_field_correlation.cpp
  //!
  //! @author mendenjl
  //! @date Jul 15, 2015
  ...
  class ExampleChemistryConformationComparisonPropertyFieldCorrelation :
    public ExampleInterface
  {
  public:

    ExampleChemistryConformationComparisonPropertyFieldCorrelation *Clone() const
    { return new ExampleChemistryConformationComparisonPropertyFieldCorrelation( *this);}
  ...
  /////////////////
    ...
    int Run() const
    {
      ...
      const double rmsd_a( chemistry::ConformationComparisonPropertyFieldCorrelation()( ensemble_vector( 1), ensemble_vector( 0)));
      BCL_MessageStd( "The RMSD between the first and second elements in the set is: " + util::Format()( rmsd_a));

      const double rmsd_b( chemistry::ConformationComparisonPropertyFieldCorrelation()( ensemble_vector( 0), ensemble_vector( 0)));
      ...
      (
        chemistry::ConformationComparisonPropertyFieldCorrelation()
        (
        ...
        1.05588,
        0.001,
        "ConformationComparisonPropertyFieldCorrelation should be able to compare constitutional-non-isomorphic molecules"
      );

      // calculate the rmsd between molecules 0 and 1 in the vector in both directions to make sure the results are
      // symmetric
      const double rmsd_1_0( chemistry::ConformationComparisonPropertyFieldCorrelation()( ensemble_vector( 1), ensemble_vector( 0)));
      const double rmsd_0_1( chemistry::ConformationComparisonPropertyFieldCorrelation()( ensemble_vector( 0), ensemble_vector( 1)));
    ...
    static const ExampleClass::EnumType ExampleChemistryConformationComparisonPropertyFieldCorrelation_Instance;

  }; //end ExampleChemistryConformationComparisonPropertyFieldCorrelation

  const ExampleClass::EnumType ExampleChemistryConformationComparisonPropertyFieldCorrelation::ExampleChemistryConformationComparisonPropertyFieldCorrelation_Instance
  (
    GetExamples().AddEnum( ExampleChemistryConformationComparisonPropertyFieldCorrelation())
  );
