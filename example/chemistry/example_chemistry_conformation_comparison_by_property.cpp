...
namespace bcl
{
  //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  //!
  //! @example example_chemistry_conformation_comparison_by_property.cpp
  //!
  //! @author mendenjl
  //! @date Sep 05, 2013
  ...
  class ExampleChemistryConformationComparisonByProperty :
    public ExampleInterface
  {
  public:

    ExampleChemistryConformationComparisonByProperty *Clone() const
    { return new ExampleChemistryConformationComparisonByProperty( *this);}
  ...
  /////////////////
    ...
    int Run() const
    {
      ...
      const double rmsd_a( chemistry::ConformationComparisonByProperty( false)( ensemble_vector( 1), ensemble_vector( 0)));
      BCL_MessageStd( "The RMSD between the first and second elements in the set is: " + util::Format()( rmsd_a));

      const double rmsd_b( chemistry::ConformationComparisonByProperty( false)( ensemble_vector( 0), ensemble_vector( 0)));
      ...
      (
        chemistry::ConformationComparisonByProperty( false)
        (
        ...
        95,
        0.001,
        "ConformationComparisonByProperty should be able to compare constitutional-non-isomorphic molecules"
      );

      // Use first MatchAndRMSD method to find the RMSD between taxol and naphthalene
      BCL_ExampleIndirectCheckWithinAbsTolerance
      (
        chemistry::ConformationComparisonByProperty( true)
        (
          taxol.GetMolecules().FirstElement(),
          napthalene.GetMolecules().FirstElement()
        ),
      ...
      // symmetric
      const double rmsd_1_0( chemistry::ConformationComparisonByProperty( false)( ensemble_vector( 1), ensemble_vector( 0)));
      const double rmsd_0_1( chemistry::ConformationComparisonByProperty( false)( ensemble_vector( 0), ensemble_vector( 1)));
      const double rmsd_1_0_tanimoto( chemistry::ConformationComparisonByProperty( true)( ensemble_vector( 1), ensemble_vector( 0)));
      const double rmsd_0_1_tanimoto( chemistry::ConformationComparisonByProperty( true)( ensemble_vector( 0), ensemble_vector( 1)));
      ...
      BCL_ExampleIndirectCheckWithinTolerance( rmsd_1_0_tanimoto, rmsd_0_1_tanimoto, 0.001, "Symmetry of MatchAndRMSD");

      return 0;
    }

    static const ExampleClass::EnumType ExampleChemistryConformationComparisonByProperty_Instance;

  }; //end ExampleChemistryConformationComparisonByProperty

  const ExampleClass::EnumType ExampleChemistryConformationComparisonByProperty::ExampleChemistryConformationComparisonByProperty_Instance
  (
    GetExamples().AddEnum( ExampleChemistryConformationComparisonByProperty())
  );
