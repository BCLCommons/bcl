...
namespace bcl
{
  //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  //!
  //! @example example_chemistry_conformation_comparison_by_real_space_rmsd.cpp
  //!
  //! @author mendenjl
  //! @date Mar 16, 2012
  ...
  class ExampleChemistryConformationComparisonByRealSpaceRmsd :
    public ExampleInterface
  {
  public:

    ExampleChemistryConformationComparisonByRealSpaceRmsd *Clone() const
    { return new ExampleChemistryConformationComparisonByRealSpaceRmsd( *this);}
  ...
  /////////////////
    ...
    int Run() const
    {
      ...
      const double rmsd_a( chemistry::ConformationComparisonByRealSpaceRmsd()( ensemble_vector( 1), ensemble_vector( 0)));
      BCL_MessageStd( "The RMSD between the first and second elements in the set is: " + util::Format()( rmsd_a));

      const double rmsd_b( chemistry::ConformationComparisonByRealSpaceRmsd()( ensemble_vector( 0), ensemble_vector( 0)));
      ...
      (
        util::IsDefined
        (
          chemistry::ConformationComparisonByRealSpaceRmsd()
          (
        ...
        false,
        "ConformationComparisonByRealSpaceRmsd should not be able to compare constitutional-non-isomorphic molecules"
      );

      // calculate the rmsd between molecules 0 and 1 in the vector in both directions to make sure the results are
      // symmetric
      const double rmsd_1_0( chemistry::ConformationComparisonByRealSpaceRmsd()( ensemble_vector( 1), ensemble_vector( 0)));
      const double rmsd_0_1( chemistry::ConformationComparisonByRealSpaceRmsd()( ensemble_vector( 0), ensemble_vector( 1)));
    ...
    static const ExampleClass::EnumType ExampleChemistryConformationComparisonByRealSpaceRmsd_Instance;

  }; //end ExampleChemistryConformationComparisonByRealSpaceRmsd

  const ExampleClass::EnumType ExampleChemistryConformationComparisonByRealSpaceRmsd::ExampleChemistryConformationComparisonByRealSpaceRmsd_Instance
  (
    GetExamples().AddEnum( ExampleChemistryConformationComparisonByRealSpaceRmsd())
  );
