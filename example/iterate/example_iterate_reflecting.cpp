...
namespace bcl
{
  //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  //
  //! @example example_iterate_Reflecting.cpp
  //! @details this examples demonstrates how to use the Reflecting iterator and times them compared to
  ...
  class ExampleIterateReflecting :
    public ExampleInterface
  {
  public:

    ExampleIterateReflecting *Clone() const
    {
      return new ExampleIterateReflecting( *this);
    }
  ...
  /////////////////
    ...
    // enum for the functions that will be tested on various Reflecting iterators
    enum Test
    {
      e_None,
      e_Validity,
    ...
    int Run() const
    {
      ...
      // that tests out a Reflecting iterator on a given container returns that it passed all the tests

      // test Reflecting iterators on an empty vector
      BCL_ExampleCheck( GetTestName( GetHighestTestPassed( storage::Vector< char>())), GetTestName( s_NumberTests));

      // test Reflecting iterators on a vector containing only 'a'
      BCL_ExampleCheck
      (
      ...
      // test Reflecting iterators on the set of vowels
      BCL_ExampleCheck
      (
      ...
      // test Reflecting iterators on a vector of chars
      BCL_ExampleCheck
      (
      ...
      // test Reflecting iterators on list of chars
      BCL_ExampleCheck
      (
        GetTestName
        (
      ...
      // normal iterator vs the Reflecting iterator
      ...
      (
        "20,000 iterations with native iterator on list",
      ...
      (
        "20,000 iterations with Reflecting iterator on list",
        util::Message::e_Standard,
        true
      );

      iterate::Reflecting< storage::Pair< double, size_t> > generic_itr_pair( pair_list.Begin(), pair_list.End());
      for( size_t i( 0), total( num_loops * pair_list.GetSize()); i < total; ++i)
      {
      ...
      iterate::Reflecting< const math::PlusEquals< float> > generic_itr_pe( plus_equals.Begin(), plus_equals.End());
      iterate::Reflecting< const math::AssignmentOperationInterface< float> >
        generic_itr_assignment( plus_equals.Begin(), plus_equals.End());

      util::Stopwatch generic_itr_time_base
      (
        "20,000 iterations with Reflecting iterator to base on vector",
        util::Message::e_Standard,
        true
      );

      for( size_t i( 0), total( num_loops * plus_equals.GetSize()); i < total; ++i)
      {
        ++generic_itr_assignment;
      }
      generic_itr_time_base.Stop();

      util::Stopwatch generic_itr_time_der
      (
        "20,000 iterations with Reflecting iterator to derived class on vector",
        util::Message::e_Standard,
        true
      );
      ...
      (
        "20,000 iterations with native iterator on vector",
        util::Message::e_Standard,
        true
      );
      for( size_t i( 0); i < num_loops; ++i)
      ...
      (
        "20,000 iterations with constructed simple pointer vector to interface",
        util::Message::e_Standard,
        true
      );
      for( size_t i( 0); i < num_loops; ++i)
      ...
      (
        ...
        added_to,
        "Reflecting iterator can handle pointer vectors without additional dereferencing"
      );
      return 0;
    }

    //! @brief a function that tests a Reflecting iterator to const char &
    //! @param CONTAINER the container to make a Reflecting iterator of chars over
    //! @return the last test passed
    //! Note: this function will be run multiple times with Reflecting iterators
    ...
    {
      // construct a new Reflecting iterator, which is a Reflecting version of the iterator returned by Begin() and End()
      // on the container
      iterate::Reflecting< const char> itr( CONTAINER.Begin(), CONTAINER.End());
      ...
      if( itr.NotAtEnd() != !CONTAINER.IsEmpty())
      {
        return highest_test_passed;
      }
      ...
      {
        return s_NumberTests;
      }
      if
      (
        size_t( itr.GetPosition()) != std::max( CONTAINER.GetSize(), size_t( 2)) - 2
        || size_t( itr.GetPosition()) != std::max( itr.GetSize(), size_t( 2)) - 2
      )
      {
        BCL_MessageStd( "Position was: " + util::Format()( itr.GetPosition()));
        return highest_test_passed;
      }
  ...
  }; //end ExampleIterateReflecting

  const ExampleClass::EnumType ExampleIterateReflecting::s_Instance
  (
    GetExamples().AddEnum( ExampleIterateReflecting())
  );
