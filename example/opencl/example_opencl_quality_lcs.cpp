...
namespace bcl
{
  //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  //!
  //! @example example_opencl_quality_lcs.cpp
  //!
  //! @author woetzen
  //! @date Apr 19, 2012
  ...
  class ExampleOpenclQualityLCS :
    public ExampleInterface
  {
  public:

    ExampleOpenclQualityLCS *Clone() const
    {
      return new ExampleOpenclQualityLCS( *this);
    }
  ...
  /////////////////
    ...
    //////////////////////////////////
      ...
      opencl::QualityLCS lcs_default( opencl::GetTools().GetFirstCommandQueue());
      ...
      opencl::QualityLCS lcs( opencl::GetTools().GetFirstCommandQueue(), rmsd_cutoff, seed_length);
      BCL_ExampleCheck( lcs.GetCutoff(), rmsd_cutoff);
      BCL_ExampleCheck( lcs.GetSeedLength(), seed_length);

      // clone construct
      util::ShPtr< opencl::QualityLCS> sp_lcs( lcs.Clone());
    ...
    /////////////////
      ...
      BCL_ExampleCheck( GetStaticClassName< opencl::QualityLCS>(), lcs.GetClassIdentifier());
    ...
    ////////////////
      ...
      const double lcs_1( opencl::QualityLCS( opencl::GetTools().GetFirstCommandQueue(), 1.0).CalculateMeasure( native_coords, model_coords));
      BCL_MessageStd( "LCS 1.0): " + util::Format()( lcs_1));
      BCL_ExampleCheck( lcs_1, expected_lcs_1);

      // calculate LCS for 2 angstrom cutoff
      const double lcs_2( opencl::QualityLCS( opencl::GetTools().GetFirstCommandQueue(), 2.0).CalculateMeasure( native_coords, model_coords));
      BCL_MessageStd( "LCS 2.0): " + util::Format()( lcs_2));
      BCL_ExampleCheck( lcs_2, expected_lcs_2);

      // calculate LCS for 5 angstrom cutoff
      const double lcs_5( opencl::QualityLCS( opencl::GetTools().GetFirstCommandQueue(), 5.0).CalculateMeasure( native_coords, model_coords));
      ...
      (
        opencl::QualityLCS( opencl::GetTools().GetFirstCommandQueue(), 2.0).CalculateSuperimposition( native_coords, model_coords).GetMatrix()
      );
  ...
  }; //end ExampleOpenclQualityLCS

  const ExampleClass::EnumType ExampleOpenclQualityLCS::s_Instance
  (
    GetExamples().AddEnum( ExampleOpenclQualityLCS())
  );
