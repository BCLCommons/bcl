...
namespace bcl
{
  ...
  //////////////////////////////////
    ...
    TMHMM::TMHMM( const linal::Vector3D &VECTOR) :
      m_Prediction( VECTOR)
    {
    }

    //! @brief Clone function
    //! @return pointer to new TMHMM
    TMHMM *TMHMM::Clone() const
    {
      return new TMHMM( *this);
    }
  ...
  /////////////////
    ...
    const std::string &TMHMM::GetClassIdentifier() const
    {
    ...
    const std::string &TMHMM::GetFileExtension() const
    {
      // static extension string
      static const std::string s_file_extension( "xxx.TMHMM");
  ...
  ////////////////
    ...
    linal::Vector3D TMHMM::GetThreeStatePrediction() const
    {
      return m_Prediction;
    }
    ...
    linal::Matrix< double> TMHMM::GetNineStatePrediction() const
    {
    ...
    std::istream &TMHMM::ReadPredictionsForAA
    (
      std::istream &ISTREAM,
    ...
    {
      // issue warning
      BCL_MessageCrt( "TMHMM ReadPredictionsForAA should not have been called");
    ...
    //! @return std::istream which was read from
    std::istream &TMHMM::ReadPredictionsForAASequence
    (
      std::istream &ISTREAM,
    ...
    ) const
    {
      ...
      static TMHMM s_default_strand_prediction( linal::Vector3D( 0.0, 1.0, 0.0));

      // static ShPtr to Method interface to be used
      static TMHMM s_default_helix_prediction( linal::Vector3D( 1.0, 0.0, 0.0));
      ...
      (
        GetMethods().e_TMHMM, AA_SEQUENCE, **GetMethods().e_TMHMM
      );
      ...
      while( ISTREAM >> ss_type)
      {
        ...
        if( ss_type == "TMhelix")
        {
          ...
          (
            GetMethods().e_TMHMM, AA_SEQUENCE, s_default_helix_prediction, start, end
          );
        }
        else if( ss_type == "TMstrand" || ss_type == "TMsheet")
        {
          MethodHandler::SetPredictionsForSubSequence
          (
            GetMethods().e_TMHMM, AA_SEQUENCE, s_default_strand_prediction, start - 1, end - 1
          );
        }
      }
  ...
  //////////////////////
    ...
    std::istream &TMHMM::Read( std::istream &ISTREAM)
    {
      // read members
    ...
    std::ostream &TMHMM::Write( std::ostream &OSTREAM, const size_t INDENT) const
    {
      // write members
