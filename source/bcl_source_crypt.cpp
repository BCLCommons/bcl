...
namespace bcl
{
  ...
  //////////
    ...
    const util::SiPtr< const util::ObjectInterface> BlowFish::s_Instance
    (
      GetObjectInstances().AddInstance( new BlowFish())
    );
  ...
  //////////////////////////////////
    ...
    {
      Reset();
      GenerateSubkeys( PASSWORD);
    }

    //! @brief Clone function
    //! @return pointer to new BlowFish
    BlowFish *BlowFish::Clone() const
    {
  ...
  /////////////////
    ...
    const std::string &BlowFish::GetClassIdentifier() const
    {
  ...
  //////////////////////
    ...
    void BlowFish::Assign( unsigned int &SET, const char &VALUE, const size_t POSITION) const
    {
      ( ( unsigned char *)( &SET))[ POSITION] = VALUE;
    }

    //! @brief convenience function to get a 1byte portion of a 4byte integer at a specific byte position
    //! @param SET integer of interest
    //! @param POSITION byte position in integer
    unsigned int BlowFish::Get( const unsigned int &SET, const size_t POSITION) const
    {
      return ( ( const unsigned char *)( &SET))[ POSITION];
    }
    ...
    void BlowFish::GenerateSubkeys( const std::string &PASSWORD)
    {
      unsigned int len( PASSWORD.size());
      unsigned int work, null0, null1;

      if( len > 0)
      {
        size_t j( 0);

        for( size_t i( 0); i < number_subkeys; ++i)
        {
          Assign( work, PASSWORD[ (j++)%len], 0);
          Assign( work, PASSWORD[ (j++)%len], 1);
          Assign( work, PASSWORD[ (j++)%len], 2);
          Assign( work, PASSWORD[ (j++)%len], 3);
          PA[ i] ^= work;
        }
        ...
        for( size_t i( 0); i < number_subkeys; i += 2)
        {
          BlowFishEncrypt( null0, null1);
          PA[ i] = null0;
          PA[ i + 1] = null1;
        }

        for( size_t j( 0); j < number_substitution_boxes; ++j)
        {
          for( size_t i( 0); i < number_entries; i += 2)
          {
            BlowFishEncrypt( null0, null1);
            SB[ j][ i] = null0;
            SB[ j][ i + 1] = null1;
    ...
    void BlowFish::BlowFishEncrypt( unsigned int &BYTES_HIGH, unsigned int &BYTES_LOW)
    {
      unsigned int word_high( BYTES_HIGH);
      unsigned int word_low( BYTES_LOW);

      word_high ^= PA[0];
      word_low ^= Cipher( word_high)^PA[1];   word_high ^= Cipher( word_low)^PA[2];
      word_low ^= Cipher( word_high)^PA[3];   word_high ^= Cipher( word_low)^PA[4];
      word_low ^= Cipher( word_high)^PA[5];   word_high ^= Cipher( word_low)^PA[6];
      word_low ^= Cipher( word_high)^PA[7];   word_high ^= Cipher( word_low)^PA[8];
      word_low ^= Cipher( word_high)^PA[9];   word_high ^= Cipher( word_low)^PA[10];
      word_low ^= Cipher( word_high)^PA[11];  word_high ^= Cipher( word_low)^PA[12];
      word_low ^= Cipher( word_high)^PA[13];  word_high ^= Cipher( word_low)^PA[14];
      word_low ^= Cipher( word_high)^PA[15];  word_high ^= Cipher( word_low)^PA[16];
      word_low ^= PA[17];
    ...
    void BlowFish::BlowFishDecrypt( unsigned int &BYTES_HIGH, unsigned int &BYTES_LOW)
    {
      unsigned int word_high( BYTES_HIGH);
      unsigned int word_low( BYTES_LOW);

      word_high ^= PA[17];
      word_low ^= Cipher( word_high)^PA[16];  word_high ^= Cipher( word_low)^PA[15];
      word_low ^= Cipher( word_high)^PA[14];  word_high ^= Cipher( word_low)^PA[13];
      word_low ^= Cipher( word_high)^PA[12];  word_high ^= Cipher( word_low)^PA[11];
      word_low ^= Cipher( word_high)^PA[10];  word_high ^= Cipher( word_low)^PA[9];
      word_low ^= Cipher( word_high)^PA[8];   word_high ^= Cipher( word_low)^PA[7];
      word_low ^= Cipher( word_high)^PA[6];   word_high ^= Cipher( word_low)^PA[5];
      word_low ^= Cipher( word_high)^PA[4];   word_high ^= Cipher( word_low)^PA[3];
      word_low ^= Cipher( word_high)^PA[2];   word_high ^= Cipher( word_low)^PA[1];
      word_low ^= PA[0];
    ...
    std::istream &BlowFish::Read( std::istream &ISTREAM)
    {
      // read members
    ...
    std::ostream &BlowFish::Write( std::ostream &OSTREAM, const size_t INDENT) const
    {
      // write members
  ...
  //////////////////////
    ...
    void BlowFish::Reset()
    {
      ...
      unsigned int PA_Init[ number_subkeys] =
      {
      ...
      unsigned int SB_Init[ number_substitution_boxes][ number_entries] =
      {
        {
      ...
      // initialize padding
      for( size_t index( 0); index < number_subkeys; ++index)
      {
        PA[ index] = PA_Init[ index];
      }

      // initialize substitution boxes
      for( size_t cols( 0); cols < number_substitution_boxes; ++cols)
      {
        for( size_t rows( 0); rows < number_entries; ++rows)
        {
          SB[ cols][ rows] = SB_Init[ cols][ rows];
        }
      }
    }

    //! @brief set password and related cryptographic tables
    //! @param PASSWORD password of interest
    void BlowFish::SetPassword( const std::string &PASSWORD)
    {
      Reset();
      GenerateSubkeys( PASSWORD);
    }
    ...
    //! @param BYTES_HIGH first four bytes of the message being encrypted
    //! @param BYTES_LOW next four bytes of the message being encrypted
    std::string BlowFish::Encrypt( const std::string &ORIGINAL_MESSAGE)
    {
      std::string encrypted_message( ORIGINAL_MESSAGE);

      // missing padding to ensure a message length divisible by eight
      const size_t padding( ( 8 - ( encrypted_message.size() % 8)) % 8);
      encrypted_message.append( padding, char( ' '));

      size_t message_size( encrypted_message.size());

      BCL_Assert( message_size % 8 == 0, "Encryption requires the input to be a multiple of 8 bytes (64 bits) to work.");

      message_size /= 8;

      // convert message into an integer pointer
      unsigned int *work( ( unsigned int *) encrypted_message.data());

      // encryption
      for( size_t i( 0); i < message_size; ++i)
      {
        unsigned int *word_high( work++);
        unsigned int *word_low( work++);

        BlowFishEncrypt( *word_high, *word_low);
      }
    ...
    //! @param BYTES_HIGH first four bytes of the message being decrypted
    //! @param BYTES_LOW next four bytes of the message being decrypted
    std::string BlowFish::Decrypt( const std::string &ENCRYPTED_MESSAGE)
    {
      std::string decrypted_message( ENCRYPTED_MESSAGE);
      size_t message_size( decrypted_message.size());

      BCL_Assert( message_size % 8 == 0, "Decryption requires the input to be a multiple of 8 bytes (64bits) to work.");

      message_size /= 8;

      // prepare integer pointer to decypted message
      unsigned int *work( ( unsigned int *) decrypted_message.data());
      ...
      for( size_t i( 0); i < message_size; ++i)
      {
        ...
        BlowFishDecrypt( *word_high, *word_low);
      }
    ...
    unsigned int BlowFish::Cipher( const unsigned int VALUE)
    {
      return ( ( SB[ 0][ Get( VALUE, 0)] + SB[ 1][ Get( VALUE, 1)]) ^ ( SB[ 2][ Get( VALUE, 2)] + SB[ 3][ Get( VALUE, 3)]));
    }
