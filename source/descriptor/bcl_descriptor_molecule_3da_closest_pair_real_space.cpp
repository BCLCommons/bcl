...
#include "descriptor/bcl_descriptor_molecule_3da_closest_pair_real_space.h"
...
namespace bcl
{
  namespace descriptor
  {
    ...
    const util::SiPtr< const util::ObjectInterface> Molecule3DAClosestPairRealSpace::s_Instance
    (
      util::Enumerated< Base< chemistry::AtomConformationalInterface, float> >::AddInstance
      (
        new Molecule3DAClosestPairRealSpace()
      )
    );
    ...
    storage::Map< std::string, storage::Pair< chemistry::FragmentComplete, storage::Map< util::ObjectDataLabel, linal::Vector< float> > > >
        Molecule3DAClosestPairRealSpace::s_Pockets =
            storage::Map< std::string, storage::Pair< chemistry::FragmentComplete, storage::Map< util::ObjectDataLabel, linal::Vector< float> > > >();
    sched::Mutex Molecule3DAClosestPairRealSpace::s_Mutex = sched::Mutex();
  ...
  //////////////////////////////////
    ...
    Molecule3DAClosestPairRealSpace::Molecule3DAClosestPairRealSpace() :
      m_NumberSteps( 14),
      m_StepSize( 0.50),
      m_Temperature( 5.0),
      m_Smooth( true),
    ...
    //! @return pointer to new Molecule3DAClosestPairRealSpace
    Molecule3DAClosestPairRealSpace *Molecule3DAClosestPairRealSpace::Clone() const
    {
      return new Molecule3DAClosestPairRealSpace( *this);
    }
  ...
  /////////////////
    ...
    const std::string &Molecule3DAClosestPairRealSpace::GetClassIdentifier() const
    {
    ...
    const std::string &Molecule3DAClosestPairRealSpace::GetAlias() const
    {
      static const std::string s_name( "3DAClosestPairRealSpace");
      return s_name;
    }

    //! @brief get number of steps of code
    //! @return number of steps in 2da code
    size_t Molecule3DAClosestPairRealSpace::GetNumberSteps() const
    {
    ...
    float Molecule3DAClosestPairRealSpace::GetStepSize() const
    {
      return m_StepSize;
    }
    ...
    const float &Molecule3DAClosestPairRealSpace::GetTemperature() const
    {
    ...
    const CheminfoProperty &Molecule3DAClosestPairRealSpace::GetAtomProperty() const
    {
  ...
  ////////////////
    ...
    //! implementations
    iterate::Generic< Base< chemistry::AtomConformationalInterface, float> > Molecule3DAClosestPairRealSpace::GetInternalDescriptors()
    {
      return
    ...
    CachePreference Molecule3DAClosestPairRealSpace::GetNormalCachePreference() const
    {
    ...
    void Molecule3DAClosestPairRealSpace::Calculate( linal::VectorReference< float> &STORAGE)
    {
      m_DiscreteCode = 0.0;
      ...
      linal::Vector< float> molecule_property( m_AtomProperty->CollectValuesOnEachElementOfObject( molecule));
      ...
      )
      {
        ...
        {
          size_t bin_dist( std::min( size_t( itr_p->Second() / m_StepSize), size_t( m_NumberSteps - 1)));
          if( bin_dist < current_min_distance)
          {
            current_min_distance = bin_dist;
            multiplicity = 1.0;
          }
          else if( bin_dist == current_min_distance)
          {
            multiplicity += 1.0;
          }
        }
        if( multiplicity)
        {
          ...
          )
          {
            ...
            size_t unsigned_bin_distance( std::min( size_t( itr_p->Second() / m_StepSize), size_t( m_NumberSteps - 1)));
            if( unsigned_bin_distance != current_min_distance)
            {
              continue;
            }
            size_t bin_distance( 3 * unsigned_bin_distance);
      ...
      // apply smoothing to our discrete bin sums
      Smooth( STORAGE);
  ...
  //////////////////////
    ...
    void Molecule3DAClosestPairRealSpace::Smooth( linal::VectorReference< float> &STORAGE) const
    {
      if( !m_Smooth)
      {
    ...
    void Molecule3DAClosestPairRealSpace::Accumulate
    (
    ...
    )
    {
      ...
      // case where the product is negative and prop_m is negative (-/+)
      if( product < 0)
      {
        sign_id = 2;
        product = -product;
      }
      // case where the product is positive and prop_m is negative (-/-)
      else if( PROP_A < 0)
      {
        sign_id = 0;
      }
      // case where the product is positive and prop_m is positive (+/+)
      else
      {
        sign_id = 1;
      }
...
//      // this was necessary for the 3DAClosestPairConvol, but it has not
...
//      }
      ...
      {
        size_t bin_distance( 3 * size_t( DISTANCE / m_StepSize));
        if( bin_distance / 3 < m_NumberSteps)
        {
    ...
    io::Serializer Molecule3DAClosestPairRealSpace::GetSerializer() const
    {
      ...
      (
        ...
        "Partial(3DAClosestPairRSAsym050(Multiply(Atom_HydrophobicTernary,Atom_Polarizability),Atom_IsInAromaticRingTernary),indices(23))",
    ...
    bool Molecule3DAClosestPairRealSpace::ReadInitializerSuccessHook
    (
    ...
    )
    {
      ...
      if( !m_MolBFilename.empty())
      {
        ...
        if( !s_Pockets.Has( m_MolBFilename))
        {
          ...
          m_PocketProperties = m_AtomProperty->CollectValuesOnEachElementOfObject( m_MolB);

          // save everything to our map of maps
          temp_map[ m_AtomProperty.GetLabel()] = m_PocketProperties;
        ...
        else
        {
          ...
          m_PocketProperties = s_Pockets.Find( m_MolBFilename)->second.Second()[ m_AtomProperty.GetLabel()];
        }
    ...
    void Molecule3DAClosestPairRealSpace::SetObjectHook()
    {
      ...
      if( m_MolBFilename.empty())
      {
        ...
        else if( !pocket_name.empty())
        {
          ...
          else
          {
            ...
            m_MolB = s_Pockets.Find( pocket_name)->second.First();
            BCL_MessageVrb( "Retrieving properties of base type: " + util::Format()( m_AtomProperty.GetLabel()));
            m_PocketProperties = s_Pockets.Find( pocket_name)->second.Second()[ m_AtomProperty.GetLabel()];
          }
