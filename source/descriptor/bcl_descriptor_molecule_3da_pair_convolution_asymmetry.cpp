...
#include "descriptor/bcl_descriptor_molecule_3da_pair_convolution_asymmetry.h"
...
namespace bcl
{
  namespace descriptor
  {
    ...
    const util::SiPtr< const util::ObjectInterface> Molecule3DAPairConvolutionAsymmetry::s_Instance
    (
      util::Enumerated< Base< chemistry::AtomConformationalInterface, float> >::AddInstance
      (
        new Molecule3DAPairConvolutionAsymmetry()
      )
    );
    ...
    storage::Map< std::string, storage::Pair< chemistry::FragmentComplete, storage::Map< util::ObjectDataLabel, linal::Vector< float> > > >
        Molecule3DAPairConvolutionAsymmetry::s_Pockets =
            storage::Map< std::string, storage::Pair< chemistry::FragmentComplete, storage::Map< util::ObjectDataLabel, linal::Vector< float> > > >();
    sched::Mutex Molecule3DAPairConvolutionAsymmetry::s_Mutex = sched::Mutex();
  ...
  //////////////////////////////////
    ...
    Molecule3DAPairConvolutionAsymmetry::Molecule3DAPairConvolutionAsymmetry() :
      m_NumberSteps( 20),
      m_StepSize( 0.50),
      m_WindowSize( 4),
    ...
    //! @return pointer to new Molecule3DAPairConvolutionAsymmetry
    Molecule3DAPairConvolutionAsymmetry *Molecule3DAPairConvolutionAsymmetry::Clone() const
    {
      return new Molecule3DAPairConvolutionAsymmetry( *this);
    }
  ...
  /////////////////
    ...
    const std::string &Molecule3DAPairConvolutionAsymmetry::GetClassIdentifier() const
    {
    ...
    const std::string &Molecule3DAPairConvolutionAsymmetry::GetAlias() const
    {
      static const std::string s_name( "3DAPairConvolutionAsymmetry");
      return s_name;
    }

    //! @brief get number of steps of code
    //! @return number of steps in 2da code
    size_t Molecule3DAPairConvolutionAsymmetry::GetNumberSteps() const
    {
    ...
    float Molecule3DAPairConvolutionAsymmetry::GetStepSize() const
    {
      return m_StepSize;
    }
    ...
    const CheminfoProperty &Molecule3DAPairConvolutionAsymmetry::GetAtomPropertyA() const
    {
    ...
    //! @brief get atom property of code
    //! @return atom property mapped in 3da code
    const CheminfoProperty &Molecule3DAPairConvolutionAsymmetry::GetAtomPropertyB() const
    {
  ...
  ////////////////
    ...
    //! implementations
    iterate::Generic< Base< chemistry::AtomConformationalInterface, float> > Molecule3DAPairConvolutionAsymmetry::GetInternalDescriptors()
    {
        return
    ...
    CachePreference Molecule3DAPairConvolutionAsymmetry::GetNormalCachePreference() const
    {
    ...
    void Molecule3DAPairConvolutionAsymmetry::Calculate( linal::VectorReference< float> &STORAGE)
    {
      //! Initial 3DA for molecule 1 property to be used in convolution
      linal::Vector< float> mol_a_3da;
      mol_a_3da = m_3DASmoothSignA.SumOverObject( *this->GetCurrentObject());
  ...
  //////////////////////
    ...
    io::Serializer Molecule3DAPairConvolutionAsymmetry::GetSerializer() const
    {
      ...
      (
        "property_a",
        "ligand property over which to calculate the function",
        io::Serialization::GetAgent( &m_AtomPropertyA)
      );
      parameters.AddInitializer
      (
        "property_b",
        "pocket property over which to calculate the function",
    ...
    bool Molecule3DAPairConvolutionAsymmetry::ReadInitializerSuccessHook
    (
    ...
    {
      // Setup 3DAs
      m_3DASmoothSignA = Molecule3DASmoothSignCode( m_AtomPropertyA, m_NumberSteps, m_StepSize, 100.0, false, true);
      m_3DASmoothSignB = Molecule3DASmoothSignCode( m_AtomPropertyB, m_NumberSteps, m_StepSize, 100.0, false, true);
      ...
      // Check that a property is provided
      if( !m_AtomPropertyA.IsDefined() || !m_AtomPropertyB.IsDefined())
      {
        return false;
      }
      ...
      if( !m_MolBFilename.empty())
      {
        ...
        if( !s_Pockets.Has( m_MolBFilename))
        {
          ...
          m_MolB3DA = ( m_3DASmoothSignB.SumOverObject( m_MolB));

          // save everything to our map of maps
          temp_map[ m_AtomPropertyB.GetLabel()] = m_MolB3DA;
    ...
    void Molecule3DAPairConvolutionAsymmetry::SetObjectHook()
    {
    ...
    void Molecule3DAPairConvolutionAsymmetry::ReduceProteinPocket( chemistry::FragmentComplete &POCKET)
    {
    }
