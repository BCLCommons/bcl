...
#include "descriptor/bcl_descriptor_molecule_3da_soft_min.h"
...
namespace bcl
{
  namespace descriptor
  {
    ...
    const util::SiPtr< const util::ObjectInterface> Molecule3DASoftMin::s_Instance
    (
      util::Enumerated< Base< chemistry::AtomConformationalInterface, float> >::AddInstance
      (
        new Molecule3DASoftMin()
      )
    );
  ...
  //////////////////////////////////
    ...
    Molecule3DASoftMin::Molecule3DASoftMin() :
      m_NumberSteps( 12),
      m_StepSize( 1.0),
      m_Temperature( 100.0),
      m_Smooth( true)
    ...
    //! @return pointer to new Molecule3DASoftMin
    Molecule3DASoftMin *Molecule3DASoftMin::Clone() const
    {
      return new Molecule3DASoftMin( *this);
    }
  ...
  /////////////////
    ...
    const std::string &Molecule3DASoftMin::GetClassIdentifier() const
    {
    ...
    const std::string &Molecule3DASoftMin::GetAlias() const
    {
      static const std::string s_name( "3daSoftMin");
      return s_name;
    }

    //! @brief get number of steps of code
    //! @return number of steps in 2da code
    size_t Molecule3DASoftMin::GetNumberSteps() const
    {
    ...
    float Molecule3DASoftMin::GetStepSize() const
    {
      return m_StepSize;
    }
    ...
    const float &Molecule3DASoftMin::GetTemperature() const
    {
    ...
    const CheminfoProperty &Molecule3DASoftMin::GetAtomProperty() const
    {
  ...
  ////////////////
    ...
    //! implementations
    iterate::Generic< Base< chemistry::AtomConformationalInterface, float> > Molecule3DASoftMin::GetInternalDescriptors()
    {
      return
    ...
    void Molecule3DASoftMin::Calculate( linal::VectorReference< float> &STORAGE)
    {
      m_DiscreteCode = math::GetHighestBoundedValue< float>();
      ...
      )
      {
        ...
        m_DiscreteCode( 0) = std::min( m_DiscreteCode( 0), prop);
        ...
        for( ++itr_atoms_b; itr_atoms_b.NotAtEnd(); ++itr_atoms_b, ++itr_prop_b)
        {
          ...
          three_da_value = std::min( prop, three_da_value);
      ...
      // for all bins for which no min was ever obtained, set it to the minimum value that was seen, provided that
      // all values seen were not identical
      const float default_value( min_max_real.GetMax() > min_max_real.GetMin() ? min_max_real.GetMax() : float( 0.0));
      ...
      )
      {
        ...
        if( *itr_3DA_rough_outer == math::GetHighestBoundedValue< float>())
        {
        ...
        {
          if( *itr_3DA_rough != math::GetHighestBoundedValue< float>())
          {
  ...
  //////////////////////
    ...
    io::Serializer Molecule3DASoftMin::GetSerializer() const
    {
    ...
    bool Molecule3DASoftMin::ReadInitializerSuccessHook
    (
      const util::ObjectDataLabel &LABEL,
      std::ostream &ERR_STREAM
    )
    {
      m_DiscreteCode = linal::Vector< float>( GetNormalSizeOfFeatures(), math::GetHighestBoundedValue< float>());
