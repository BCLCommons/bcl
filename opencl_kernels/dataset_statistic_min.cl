...
//! @brief determine the minimal value of each vector elements in a data set
...
)
{
  ...
  {
    accumulator = min( accumulator, DATASET[ global_index * VECTOR_LENGTH + vector_index]);
  ...
  for( size_t offset = get_local_size( 0) / 2; offset > 0; offset = offset / 2)
  {
    ...
    {
      SHARED[ local_index] = min( SHARED[ local_index + offset], SHARED[ local_index]);
    }
    barrier( CLK_LOCAL_MEM_FENCE);
  }
  if( local_index == 0)
